...
$app->group('/purchase', function (RouteCollectorProxy $app) {
  ...
  $app->post('/{id}/~clear-all-received',
              [ \Scat\Controller\Transactions::class, 'clearAllReceived' ]);
...
$app->group('/catalog', function (RouteCollectorProxy $app) {
  ...
  $app->post('/~print-labels',
              [ \Scat\Controller\Catalog::class, 'printLabels' ]);
...
$app->get('/{path:.*}', function (Request $request, Response $response, $path) {
  if (preg_match('/\.(js|css|ttf|png|svg|eot|woff|woff2)$/', $path, $m) &&
      file_exists("../" . $path))
  {
    $fp= fopen("../" . $path, 'r');
    $types= [
