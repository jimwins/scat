...
if ($line) {
  $q= "UPDATE txn_line SET ordered = allocated WHERE txn_id = $id AND id = $line";
  ...
  $q= "UPDATE txn_line SET ordered = allocated WHERE txn_id = $id";
  $r= $db->query($q)
    or die_jsonp($db->error);
  $lines= $db->affected_rows;
...
echo jsonp(array("success" => "Closed all lines.",
                 "txn" => $txn,
                 "lines" => $lines,
